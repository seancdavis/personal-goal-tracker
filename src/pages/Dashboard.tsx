...
export function Dashboard() {
  const fetchWeeks = useCallback(() => weeksApi.list(), []);
  const weeks = useAsyncData(fetchWeeks);

  const sortedWeeks = weeks.data
    ? [...weeks.data].sort((a, b) => b.id.localeCompare(a.id))
    : [];

  const currentWeekId = getCurrentWeekId();
  const currentWeek = sortedWeeks.find((w) => w.id === currentWeekId);
  ...
  return (
    <div className="space-y-6">
      ...
      <AsyncSection
        ...
        loadingElement={<SkeletonCard className="overflow-hidden" />}
      >
        {() =>
          currentWeek ? (
...
function WeekRow({ week, isCurrent }: { week: Week; isCurrent: boolean }) {
